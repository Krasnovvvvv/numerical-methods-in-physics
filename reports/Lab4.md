...
2. [Структура проекта 🛠️](#2-структура-проекта-🛠️)
3. [Ход выполнения работы 🔎](#3-ход-выполнения-работы-🔎)
    - [Реализация численных методов 🚀](#реализация-численных-методов-🚀)
        - [Классический метод Эйлера ✏️](#классический-метод-эйлера-✏️)
        - [Улучшенный метод Эйлера ⚙️](#улучшенный-метод-эйлера-⚙️)
...
```
Реализация поддерживает автоматическое добавление новых заданий, централизованное подключение заголовков и простое масштабирование под разные лабораторные работы. Реализацию. всех методов можно найти по пути `include/Labs/Lab4/ODESolvers`
...
#### Улучшенный метод Эйлера ⚙️

Для повышения точности используется _усовершенствованный метод Эйлера_, _(Метод Эйлера-Коши, предиктор-корректор)_, который также является явным, но реализует двухшаговый принцип _предиктора-корректора_:
...
Решим уравнение Эйлера методом Рунге-Кутты 4 порядка и сравним полученный результат с аналитическим решением:

![Решение уравнения Эйлера](../images/Lab4/RungeKutta4SolutionVSAnalyticSolutionZoomed.png)


**График 8 - Сравнение численного и аналитического решений уравнения Эйлера методом Рунге-Кутты**

Как видим, при оптимальном подборе шага численное решение полностью совпадает с аналитическим. Отметим, что решение улучшенным методом Эйлера дает тот же результат, но различия всё же есть. Существенно различаются ошибки, допускаемые данными методами. Построим графики оценки ошибки по сравнению с аналитическим решением:

![Ошибка решения по Эйлеру](../images/Lab4/ImprovedEulerErrorVSAnalyticSolution.png)


**График 9 - Оценка ошибки решения по улучшенному методу Эйлера**

---

![Ошибка решения по Рунге-Кутты](../images/Lab4/RungeKutta4ErrorVSAnalyticSolution.png)


**График 10 - Оценка ошибки решения по методу Рунге-Кутты**

Графики подтверждают высокую точность метода Рунге-Кутты 4 порядка - порядок ошибки остается неизменным и равным $10^{-6}$. Рост ошибки со временем является классическим поведением для данных численных методов

---

Теперь сравним решения различными методами при одном и том же шаге $h = 10^{-2}$:

![Решение уравнения Эйлера различными методами](../images/Lab4/DifferentSolversSolutionOfOscillatorWithStep1e-3.png)


**График 11 - Решение уравнения Эйлера различными методами при фиксированном шаге**

Заметим, что классический метод Эйлера сильно завышает реальные численные значения координаты, а значит является неоптимальным для решения данной задачи

---

Осталось самое интересное, фазовые портреты! Приведем фазовые портреты, полученные в результате решения различными численными методами при шаге $h = 10^{-2}$:

![Фазовые портреты](../images/Lab4/PhaseTrajectoryOfOscillatorByDifferentSolvers.png)


**График 12 - Фазовые портреты, полученные при решении различными методами**

Полученные фазовые портреты полностью согласуются с ожидаемыми. Для затухающих колебаний характерны фазовые портреты с типом особой точки "устойчивый фокус", который у нас и получился :)

А как будет влиять шаг на фазовый портрет? Проведем эксперимент для метода Рунге-Кутты:

![Фазовые портреты при разном шаге](../images/Lab4/PhaseTrajectoryOfOscillatorByRungeKutta4WithDifferentStep.png)


**График 13 - Фазовые портреты, полученные при решении методом Рунге-Кутты при различном шаге**

Таким образом, шаг существенно влияет на итоговое численное решение, даже несмотря на использование довольно точных методов, таких как метод Рунге-Кутты

---

Приведем графики зависимости координаты $x$ от времени $t$ при различных параметрах системы:

Пусть коэффициент $b = 2.5$

![Решение при другом b](../images/Lab4/RungeKutta4XOfOscillatorWithB2.5.png)


**График 14 - Зависимость координаты x от времени t при параметре $b = 2.5$**

Параметр $b$ главным образом отвечает за "быстроту" затухания, что мы и видим на полученном графике: довольно быстрое затухание по сравнению с системой, для которой $b = 1.0$

Пусть частота колебаний $\omega = 3.0$

![Решение при другом w](../images/Lab4/RungeKutta4XOfOscillatorWithOmega3.0.png)


**График 15 - Зависимость координаты x от времени t при параметре $\omega = 3.0$**

Заметно существенное замедление колебаний по сравнению с системой, для которой $\omega = 10.0$

---

Проведенное исследование поведения решений уравнения Эйлера-Коши с использованием различных численных методов позволяет сделать следующие выводы:

- **Совпадение численного и аналитического решений:**

На графике сравнения численного и аналитического решений (График 8) видно, что при выборе небольшого шага интегрирования и использовании метода Рунге-Кутты 4-го порядка, полученное численное решение практически полностью совпадает с эталонным аналитическим решением. Это подтверждает высокую точность метода для данной задачи

- **Ошибка численных методов:**

Графики ошибок (Графики 9 и 10) показывают, что метод Рунге-Кутты 4-го порядка обеспечивает ошибку на уровне $10^{-6}$, а улучшенный метод Эйлера — порядка $0.01$. Существенный рост ошибки наблюдается в начальный период и стабилизируется со временем, что согласуется с классическими представлениями о накоплении глобальной ошибки при численном интегрировании

- **Сравнение методов на одном шаге:**

Сравнение решений различными численными методами (График 11) на фиксированном шаге $h=0.01$ демонстрирует существенное превышение классическим методом Эйлера реальных значений координаты. Методы повышенного порядка (улучшенный Эйлер и РК4) дают более точные и устойчивые траектории

- **Анализ фазовых портретов:**

Фазовые портреты (Графики 12 и 13) полностью подтверждают ожидаемый тип особой точки "устойчивый фокус" для затухающего осциллятора. При уменьшении шага точность восстановления фазовой траектории существенно возрастает даже для метода Рунге-Кутты

- **Влияние параметров системы:**

По графикам зависимости координаты от времени, видно, что увеличение параметра $b$ приводит к более быстрому затуханию колебаний (График 14), а уменьшение частоты $\omega$ — к заметному замедлению процесса (График 15)
