...
— безразмерное число Фурье, определяющее относительную скорость теплопроводности относительно временного масштаба импульса. 

Граничные условия переписываются как:

$$
-\frac{\partial \theta}{\partial \xi}(0,\tau) = \exp(-\tau),\qquad \frac{\partial \theta}{\partial \xi}(1,\tau)=0,\qquad \theta(\xi,0)=0.
$$

Таким образом, динамика нагрева описывается всего двумя безразмерными параметрами: числом Фурье $\mathrm{Fo}$ и отношением интенсивности излучения к теплопроводности, свернутым в масштаб температуры $\Delta T$.

---

#### Аппроксимация граничных условий

В безразмерных переменных граничные условия имеют вид:

$$
\theta_{\xi}(0,\tau) = - e^{-\tau}, \qquad \theta_{\xi}(1,\tau) = 0
$$

##### Левый Нейман (2‑й порядок)

Для левого конца $\xi = 0$ используем одностороннюю аппроксимацию производной второго порядка:

$$
-\theta_{\xi}(0,\tau_{s+1}) \approx -\frac{-3\theta_0^{s+1} + 4\theta_1^{s+1} - \theta_2^{s+1}}{2h} =  e^{-\tau_{s+1}}
$$

Отсюда получаем разностное граничное условие:

$$
3\theta_0^{s+1} - 4\theta_1^{s+1} + \theta_2^{s+1} = 2h e^{-\tau_{s+1}}
$$

##### Правый Нейман (1‑й порядок)

На правом конце $\xi = 1$ условие теплоизоляции аппроксимируется односторонней разностью первого порядка:

$$
\theta_{\xi}(1,\tau_{s+1}) \approx \frac{\theta_N^{s+1} - \theta_{N-1}^{s+1}}{h} = 0 \;\Rightarrow\; \theta_N^{s+1} = \theta_{N-1}^{s+1}
$$
...
- $k = 50 \text{Вт/(м·К)}$ — коэффициент теплопроводности
- $l = 0,01 \text{м}$ — длина стержня
- $t_p = 0,01 \text{с}$ — характерное время лазерного импульса
- $I_0 = 1,0 \cdot 10^{5}\ \text{Вт/м}^2$ — интенсивность лазерного потока на левом конце
...
На графике зависимости температуры в точке $\xi=0$ от времени видно монотонное ростовое поведение: температура на освещённом конце стержня быстро возрастает от начального значения $T_0$ до приблизительно $390$ K на интервале моделирования. Изначально рост почти линейный, затем, по мере затухания экспоненциального лазерного импульса, нагрев замедляется, но за счёт теплопроводности часть тепла продолжает перетекать в глубину стержня, что сглаживает кривую, придавая ей “квазипараболический” вид.

Распределения температуры вдоль стержня для нескольких моментов времени показывают тонкий нагретый слой вблизи $\xi=0$, ширина которого со временем растёт. На ранних стадиях максимум температуры локализован практически на самом левом конце, а уже на более поздних временах прогретая область углубляется на несколько шагов сетки, при этом температура вдоль стержня остаётся существенно ниже температуры на границе. Форма профиля напоминает быстро спадающую экспоненту, что естественно для балансировки локального потока тепла и диффузионного переноса вглубь материала.
