...
4. [Заключение и выводы](#5-заключение-и-выводы-)
...
Требуется найти корень уравнения $$2\ln(x) - \cos(\ln(x)) + \sin(\ln(x)) = 0$$ на отрезке $$[1, 3]$$. Приведем сравнительную таблицу для всех методов при $$step = 10^-5, esp = 10^-12$$:
...
- Для каждой фиксированной \( V \) ищется корень \( T \) вышеуказанного нелинейного уравнения (через root-solver)
