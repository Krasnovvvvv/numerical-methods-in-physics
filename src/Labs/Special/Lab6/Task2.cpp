/// Задача 2: Отражение от бесконечного диэлектрика (кварц, n=1.45)
///
/// Графики:
/// 1. R(λ) из FDTD + горизонтальная линия теории Френеля R = ((1-n)/(1+n))²
...
int main() {
    ...
    const double c0         = 3e8;          // м/с
    const double lambda_min = 300e-9;       // 300 нм
    const double lambda_max = 900e-9;       // 900 нм
    const double n_quartz   = 1.45;
    const double eps_quartz = n_quartz * n_quartz;

    // Сетка
    const double dx    = 10e-9;             // 10 нм
    const int pml_N    = 40;
    ...
    // Кварц: от интерфейса до конца (включая правый PML)
    ...
    double T_theory = 1.0 - R_theory;  // = 4n/(1+n)^2

    // Поправка на импеданс: T_power = n * |E_trans/E_inc|^2
    // (монитор находится внутри диэлектрика)
    ...
    std::cout << "\nTheoretical R = " << R_theory << "\n";
    std::cout << "Theoretical T = " << T_theory << "\n";
    std::cout << "\nTask 2 complete.\n";
    std::cout << "Ожидание: R(λ) ≈ const ≈ 0.0337, T(λ) ≈ const ≈ 0.9663\n";
}