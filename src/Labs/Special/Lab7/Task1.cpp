...
namespace {
    ...
    double exactTask1(double x, double y) {
        if (y > x) return 1.0;
        if (y < x) return 0.0;
        return 0.5;
    }

    AdvectionDiffusionTaskConfig makeTask1Config(double gamma,
        ...
        cfg.left   = BoundaryCondition::dirichlet(1.0);
    ...
    std::vector<double> exactOnSegmentTask1(size_t points) {
...
int main() {
    ...
    auto mainCfg = makeTask1Config(
        0.0,
        ConvectionScheme::TVD,
        TVDLimiterType::VanLeer,
        "Task 1: c(x,y), Gamma = 0, TVD-VanLeer",
        true,
        true
    );
    ...
    auto upwindCfg = makeTask1Config(
        0.0,
        ConvectionScheme::Upwind,
        TVDLimiterType::VanLeer,
        "Task 1: Upwind, Gamma = 0"
    );
    ...
    curves.push_back(exactOnSegmentTask1(LinePoints));
    ...
    for (auto lim : tvdLimiters) {
        auto cfg = makeTask1Config(
            0.0,
            ConvectionScheme::TVD,
            lim,
            "Task 1: " + AdvectionDiffusionTask::schemeName(ConvectionScheme::TVD, lim) + ", Gamma = 0"
        );
    ...
    CFDPlotter::plotCurves(
        ...
        curves,
        labels,
        "Task 1: comparison on line (0,1)-(1,0), Gamma = 0",
        "t on line: (x,y) = (t, 1 - t)",
        "c"
    );

    auto diffCfg1 = makeTask1Config(
        1e-3,
        ConvectionScheme::TVD,
        TVDLimiterType::VanLeer,
        "Task 1: c(x,y), Gamma = 1e-3, TVD-VanLeer",
        true,
        false
    );
    ...
    auto diffCfg2 = makeTask1Config(
        1e-2,
        ConvectionScheme::TVD,
        TVDLimiterType::VanLeer,
        "Task 1: c(x,y), Gamma = 1e-2, TVD-VanLeer",
        true,
        false
    );
    ...
    CFDPlotter::plotCurves(
        ...
        "Task 1: effect of nonzero diffusion on line (0,1)-(1,0)",
        "t on line: (x,y) = (t, 1 - t)",
        "c"
    );
