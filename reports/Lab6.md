...
— число Фурье, определяющее относительную скорость теплопроводности относительно временного масштаба импульса.

Граничные условия переписываются как:

$$
-\frac{\partial \theta}{\partial \xi}(0,\tau) = \beta\tau e^{-\tau},\qquad \beta = \frac{I_0 l}{k \Delta T} = \frac{\rho c l^2}{k t_p} = \frac{1}{\mathrm{Fo}}
$$
...
Таким образом, динамика нагрева описывается двумя безразмерными параметрами: числом Фурье $\mathrm{Fo}$ и коэффициентом $\beta = 1/\mathrm{Fo}$, связанным с выбором масштаба температуры $\Delta T$.
...
На левом конце $\xi = 0$ используем одностороннюю аппроксимацию производной второго порядка:

$$
-\theta_{\xi}(0,\tau_{s+1}) \approx -\frac{-3\theta_0^{s+1} + 4\theta_1^{s+1} - \theta_2^{s+1}}{2h} = \beta\tau_{s+1} e^{-\tau_{s+1}}
$$

Отсюда получаем разностное граничное условие:

$$
3\theta_0^{s+1} - 4\theta_1^{s+1} + \theta_2^{s+1} = 2h\beta\tau_{s+1} e^{-\tau_{s+1}}
$$
